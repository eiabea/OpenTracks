...
public class ImportService extends JobIntentService {
    ...
    private void importFile(DocumentFile file) {
        ...
        try (InputStream inputStream = getContentResolver().openInputStream(file.getUri())) {
            ...
            if (!trackIds.isEmpty()) {
                sendResult(ImportServiceResultReceiver.RESULT_CODE_IMPORTED, trackIds, file.getName(), getString(R.string.import_file_imported, file.getName()));
            } else {
                sendResult(ImportServiceResultReceiver.RESULT_CODE_ERROR, trackIds, file.getName(), getString(R.string.import_unable_to_import_file, file.getName()));
            }
        } catch (IOException e) {
    ...
    private void sendResult(int resultCode, ArrayList<Track.Id> trackId, String fileName, String message) {
